  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        console.log('Data Error in Write Article: ', data.message);
        toast.error(data.message)
      }

    } catch (error) {
      console.log('Error in Write Article: ', error.message);
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {articleLength.map((item, index) => (
            <span
              onClick={() => setSelectedLength(item)} key={index} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength.text === item.text ? 'bg-blue-50 text-blue-700' : 'text-gray-500 border-gray-300'}`}>
              {item.text}
            </span>
          ))}
        </div>
